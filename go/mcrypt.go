...
char* encrypt(void* key, int keyLength, void* iv, int ivLength, char* data, int* length, char* calgo, char* cmode, int* err) {
	if (*length <= 0) {
		*err = INVALID_DATA_LENGTH;
		return NULL;
	}
	int i;
	MCRYPT td = mcrypt_module_open(calgo, NULL, cmode, NULL);
	if (td == MCRYPT_FAILED) {
...
char* decrypt(void* key, int keyLength, void* iv, int ivLength, char* data, int* length, char* calgo, char* cmode, int* err) {
	int i;
	MCRYPT td = mcrypt_module_open(calgo, NULL, cmode, NULL);
	if (td == MCRYPT_FAILED) {
		*err = FAILED_MCRYPT_MODULE;
